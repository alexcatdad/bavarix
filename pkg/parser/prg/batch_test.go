...
func TestBatchExtractAllFilesProcessed(t *testing.T) {
	...
	results, err := BatchExtract(path)
	require.NoError(t, err)
	// We have C_GM5.prg and LSZ.prg in testdata
	assert.Len(t, results, 2, "should process both test PRG files")
...
func TestBatchResultStats(t *testing.T) {
	...
	stats := results.Stats()
	assert.Equal(t, 2, stats.TotalFiles, "should have 2 total files")
	assert.Equal(t, 2, stats.SuccessCount, "both files should parse successfully")
